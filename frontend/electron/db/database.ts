...
function runMigrations(): void {
  ...
  try { db.exec('ALTER TABLE projects ADD COLUMN local_path TEXT NOT NULL DEFAULT \'\';'); } catch { }
  try { db.exec('ALTER TABLE projects DROP COLUMN sonar_project_key;'); } catch { }
  try { db.exec('ALTER TABLE projects DROP COLUMN sonar_token;'); } catch { }
  try { db.exec('ALTER TABLE projects DROP COLUMN sonar_url;'); } catch { }
  ...
  try { db.exec('ALTER TABLE tasks ADD COLUMN profile_id TEXT REFERENCES agent_profiles(id) ON DELETE SET NULL;'); } catch { }
  try { db.exec('ALTER TABLE tasks ADD COLUMN branch_name TEXT;'); } catch { }
  try { db.exec('ALTER TABLE tasks ADD COLUMN turn_count INTEGER DEFAULT 0;'); } catch { }
  try { db.exec('ALTER TABLE tasks ADD COLUMN input_tokens INTEGER DEFAULT 0;'); } catch { }
  try { db.exec('ALTER TABLE tasks ADD COLUMN output_tokens INTEGER DEFAULT 0;'); } catch { }
  try { db.exec('ALTER TABLE tasks ADD COLUMN elapsed_seconds INTEGER DEFAULT 0;'); } catch { }
  try { db.exec('ALTER TABLE tasks ADD COLUMN queued_at TEXT;'); } catch { }
  ...
  try { db.exec('ALTER TABLE agents ADD COLUMN skip_permissions INTEGER DEFAULT 0;'); } catch { }
